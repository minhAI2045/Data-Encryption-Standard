...
plaintext_binary, key_binary = convert_text_to_binary(plaintext, key)
print(f"Plaintext là: {plaintext} có len {len(plaintext)}")
print(f"Key là: {key} có len {len(key)}")
print(f"Plaintext là: {plaintext_binary} có len {len(plaintext_binary)}")
print(f"Key là: {key_binary} có len {len(key_binary)}")
# Bước 1: IP plaintext
plaintext_binary = initial_permutation(plaintext_binary)
print(f"Plaintext sau biến đổi IP là: {plaintext_binary} có len {len(plaintext_binary)}")
# Bước 2: Key-depend computation
KS = key_schedule(key_binary)
print("KS là", KS)
plaintext_binary = key_depend_computation(plaintext_binary, KS)
print(f"Plaintext sau bước 2 là: {plaintext_binary} có len {len(plaintext_binary)}")
# Bước 3: IP-1 plaintext
plaintext_binary = inverse_initital_permutation(plaintext_binary)
print(f"Plaintext sau bước 3 là: {plaintext_binary} có len {len(plaintext_binary)}")
ciphertext = convert_binary_to_hex(plaintext_binary)
print(f"Ciphertext sau khi mã hóa: {ciphertext}")
